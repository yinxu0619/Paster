...
对标 [Paste (pasteapp.io)](https://pasteapp.io) 的 macOS 原生剪贴板历史工具。后台实时记录剪贴板，全局热键呼出面板，支持文本 / 富文本 / 图片 / 文件 / 链接，本地存储、不联网。
...
- **呼出位置可选**：跟随光标悬浮、屏幕底部 / 顶部 / 左侧 / 右侧 / 中央。
  - 选「底部 / 顶部」时为 **Paste 式全宽横向平铺条**，从屏幕边缘升起；
  - 横向条高度可通过**设置里的滑块（带可视化预览）**或**直接拖拽面板上边缘**调整，卡片内容随高度自适应放大。
- **多模式粘贴**：保留原格式粘贴、纯文本粘贴（剥离格式）、重新复制；卡片右键菜单与键盘操作。
- **搜索与分类**：顶部实时关键词搜索、来源应用筛选；Pin 固定独立分组（Pinboard）置顶。
- **键盘全操作**：`↑↓`（横向条为 `←→`）选择、回车粘贴、`⌘⌫` 删除、`⌘P` 固定、`⌘Y` 全屏预览、`Esc` 关闭，呼出后搜索框自动聚焦。
